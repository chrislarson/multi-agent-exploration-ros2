...
def generate_launch_description():
    ...
    launch_dir_2 = os.path.join(package_dir, "launch")
    ...
    declare_world_cmd = DeclareLaunchArgument(
        "world",
        default_value=os.path.join(package_dir, "worlds", "world_only.model"),
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        "params_file",
        default_value=os.path.join(package_dir, "params", "nav2_multirobot_params_all.yaml"),
        ...
        group = GroupAction(
            [
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(os.path.join(launch_dir_2, "rviz_launch.py")),
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(os.path.join(package_dir, "launch", "tb3_simulation_launch.py")),
                    launch_arguments={
    ...
    ld.add_action(LogInfo(condition=IfCondition(log_settings), msg=["map yaml: ", map_yaml_file]))
    ld.add_action(LogInfo(condition=IfCondition(log_settings), msg=["params yaml: ", params_file]))
    ld.add_action(LogInfo(condition=IfCondition(log_settings), msg=["rviz config file: ", rviz_config_file]))
    ld.add_action(LogInfo(condition=IfCondition(log_settings), msg=["using robot state pub: ", use_robot_state_pub]))
    ld.add_action(LogInfo(condition=IfCondition(log_settings), msg=["autostart: ", autostart]))
